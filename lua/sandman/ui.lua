...
function M.show_inspector(bufnr, block_id)
    ...
    if
        not M.inspector_win or not vim.api.nvim_win_is_valid(M.inspector_win)
    then
        ...
        vim.api.nvim_buf_set_keymap(M.inspector_buf, "n", "Q", "", {
            ...
            callback = function()
                if
                    M.inspector_win
                    and vim.api.nvim_win_is_valid(M.inspector_win)
                then
...
function M.render_inspector(state, focus_block_id)
    if
        not M.inspector_buf or not vim.api.nvim_buf_is_valid(M.inspector_buf)
    then
        return
    end
    ...
    for block_id, requests in pairs(state.requests) do
        if #requests > 0 then
            ...
            for i, req in ipairs(requests) do
                ...
                table.insert(
                    lines,
                    string.format("### Request %d: `%s` %s", i, method, url)
                )
                table.insert(lines, "")
    ...
    if total_requests == 0 then
        table.insert(
            lines,
            "> No requests yet. Run a block with HTTP calls to see them here."
        )
    end
...
function M.toggle_log(state)
    if M.log_win and vim.api.nvim_win_is_valid(M.log_win) then
        ...
        local opts = config.get()
        M.log_win, M.log_buf =
            create_split("Sandman Log", opts.log_position, opts.log_size)
        M.render_log(state)
    end
end
...
function M.show_output(state, block_id, block)
    ...
    M.output_win, M.output_buf = create_float(
        string.format(" Block %d Output ", block_id),
        width,
        height,
        row,
        col
...
function M.render_output(state, block_id, block)
    ...
    table.insert(
        lines,
        string.format("# Block %d - %s", block_id, block_state:upper())
    )
    table.insert(lines, "")
